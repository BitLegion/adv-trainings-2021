...
bool Drivetrain::block_and_connect() {
    ...
    if (socket_fd_ == -1) {
        std::cerr << "Error creating a socket: " << std::strerror(errno) << std::endl;
        return false;
    }
    ...
    int result = connect(socket_fd_, (const sockaddr *)&addr, sizeof(sockaddr_un)); 
    ...
    if (result == -1) {
        std::cerr << "Failed to connect on drivetrain socket: " << std::strerror(errno) << std::endl;
        return false;
    }
...
bool Drivetrain::iterate() {
    ...
    if (result == -1) {
        std::cerr << "Failed to read on drivetrain socket: " << std::strerror(errno) << std::endl;
        return false;
    } else if (result == 0) {
    ...
    if (result == -1) {
        std::cerr << "Failed to write on drivetrain socket: " << std::strerror(errno) << std::endl;
        return false;
    } else if (result == 0) {
